...
const newCategory = function(categoryName, config) {
  ...
  if (config.syslog) {
    ...
    //  //host:      config.syslog.host || localhost,
    //  //port:      config.syslog.port || 514,
    //  //protocol:  config.syslog.protocol || 'udp4',
    //  //path:      config.syslog.path || ' /dev/log',
    //  //pid:       config.syslog.pid || process.pid,
    //  //facility:  config.syslog.facility || 'local0',
    //  //localhost: config.syslog.localhost || 'localhost',
    //  //type:      config.syslog.localhost || 'localhost',
    //  //app_name:  config.syslog.app_name || process.title,
    //  //eol:       config.syslog.eol || ,
    //});
